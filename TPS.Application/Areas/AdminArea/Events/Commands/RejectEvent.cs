...
namespace TPS.Application.Areas.AdminArea.Events.Commands
{
    public class  RejectEvent
    {
        public sealed class Command : ICommand<Result>
        {
            ...
            public Command(Guid userId, Guid eventId, string remark)
            {
                UserId = userId;
                EventId = eventId;
                Remark = remark;
            }
            public static Command Create(Guid userId, Guid eventId, string remark) => new Command(userId, eventId, remark);
        }
        public sealed class Handler : ICommandHandler<Command, Result>
        {
            ...
            public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                {
                    eventStatus.DeanAssistantApproval = false;
                    eventStatus.Remarks = request.Remark;
